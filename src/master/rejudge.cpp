...
int UpdatorJob::rejudge(const Result & result)
{
    ...
    mysqlpp::Query query = mysqlConn.query(
            "select s_result, s_time, s_mem, s_similarity_percentage from %0 where s_id = %1");
    query.parse();
    ...
    try {
        insert_rejudge_solution_templ.store(jobType, sid, orig_result, std::stoi(row[1]), std::stoll(row[2]),
                                            std::stoi(row[3]));
...
int UpdatorJob::rejudge_solution(const Result & result)
{
    std::string solution_table;
    if (!jobType) {
        solution_table = "solution";
    ...
    MysqlRes query_res;
    try {
        query_res = cmd.execute(mysqlConn, solution_table, (int) result.judge_result, result.cpu_time.count(),
...
int UpdatorJob::rejudge_user_and_problem(MYSQL * conn, const Result & _result, UnitedJudgeResult orig_result)
{
    ...
    sprintf(sql, "select count(case when s_posttime < '%s' and s_result = '0' then 1 end), "
                 ...
                 "from solution where c_id %s and u_id = '%d' and p_id = '%d'", postTime.c_str(), postTime.c_str(),
            postTime.c_str(), cid_param, uid, pid);
    int ret = mysql_real_query(conn, sql, strlen(sql));
    if (ret) {
        LOG_WARNING(jobType, sid, log_fp,
                    "REJUDGE: select count info from solution failed!, errno:%d, error:%s\nsql:%s", mysql_errno(conn),
    ...
    if (acbefore == 0
        ...
        if (acafter > 0) { //如果在rejudge的solution后有原来AC的solution
            ...
            sprintf(sql, "select count(*) from solution "
                         ...
                         "and c_id %s and u_id = '%d' and p_id = '%d'", postTime.c_str(), cid_param, uid, pid,
                    postTime.c_str(), cid_param, uid, pid);
            ...
            //更新user表
            if (_result.judge_result == UnitedJudgeResult::ACCEPTED) {
                sprintf(sql, "update user set u_submit = u_submit - %d, u_accept = u_accept + 1 where u_id = '%d'",
                        submitafter, uid);
            } else {
                sprintf(sql, "update user set u_submit = u_submit + %d, u_accept = u_accept - 1 where u_id = '%d'",
                        submitafter, uid);
            ...
            //更新problem表
            if (_result.judge_result == UnitedJudgeResult::ACCEPTED) {
                sprintf(sql, "update problem set p_submit = p_submit - %d, p_accept = p_accept + 1 where p_id = '%d'",
                        submitafter, pid);
            } else {
                sprintf(sql, "update problem set p_submit = p_submit + %d, p_accept = p_accept - 1 where p_id = '%d'",
                        submitafter, pid);
    ...
    if (acbefore == 0
        ...
        if (acafter > 0) {
            ...
            sprintf(sql, "select count(*) from solution "
                         ...
                         "and c_id = '%d' and u_id = '%d' and p_id = '%d'", postTime.c_str(), cid, uid, pid,
                    postTime.c_str(), cid, uid, pid);
            ret = mysql_real_query(conn, sql, strlen(sql));
            if (ret) {
                LOG_WARNING(jobType, sid, log_fp,
                            "REJUDGE: select count info from solution failed!, errno:%d, error:%s\nsql:%s",
            ...
            if (result.judge_result == UnitedJudgeResult::ACCEPTED) {
                sprintf(sql, "update course_user set c_submit = c_submit - %d - 1 where c_id = '%d' and u_id = '%d'",
                        atoi(row[0]), cid, uid);
            } else {
                sprintf(sql, "update course_user set c_submit = c_submit + %d + 1 where c_id = '%d' and u_id = '%d'",
                        atoi(row[0]), cid, uid);
            ...
            if (result.judge_result == UnitedJudgeResult::ACCEPTED) {
                sprintf(sql,
                        "update course_user set c_submit = c_submit - %d, c_accept = c_accept + 1 where c_id = '%d' and u_id = '%d'",
                        submitafter, cid, uid);
            } else {
                sprintf(sql,
                        "update course_user set c_submit = c_submit + %d, c_accept = c_accept - 1 where c_id = '%d' and u_id = '%d'",
