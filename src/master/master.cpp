...
int main()
{
	...
	while (loop) {
		...
		try {
			...
			std::tie(type, update_id) = JobBase::parserJobItem(job_item);
		...
		try {
			job.fetchDetailsFromRedis(redis_conn);
		} catch (...) {
			ret2 = -1;
		}
		Result result;
		try {
			result = job.get_job_result(redis_conn);
		} catch (...) {
			ret3 = -1;
		}
		...
		if (job.is_rejudge) {
			if (job.rejudge(&mysql_conn, result)) {
			...
			if (result.judge_result == UnitedJudgeResult::ACCEPTED) {
				...
				if (reply2 && reply2.replyType() == RedisReplyType::STRING)  //取到则说明该用户在同一时间内ac两题
				{
			...
			job.set_cheat_status(redis_conn, EXPIRE_TIME);
			ret1 = job.update_solution(&mysql_conn, result);
			ret2 = job.update_source_code(&mysql_conn, reply->str);
			if (!type) {
				if (job.cid > 0) {
					ret3 = job.update_course_user(&mysql_conn, result);
				}
				ret4 = job.update_user_and_problem(&mysql_conn, result);
				ret5 = job.update_user_problem(&mysql_conn, result.judge_result == UnitedJudgeResult::ACCEPTED ? 0 : 1);
			} else {
				ret3 = job.update_contest_user_problem(&mysql_conn, result.judge_result == UnitedJudgeResult::ACCEPTED ? 1 : 0);
			}
		}

		//保存编译信息
		if (result.judge_result == UnitedJudgeResult::COMPILE_ERROR) {
			ret6 = job.update_compile_info(&mysql_conn, redis_conn);
			if (ret6) {
		...
		if (ret1 || ret2 || ret3 || ret4 || ret5 || ret6) {
			...
			job.core_update_failed_table(&mysql_conn, redis_conn, result);
		}

		job.commitJudgeStatusToRedis(redis_conn, JudgeStatus::UPDATED);
