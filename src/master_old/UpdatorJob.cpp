...
void UpdatorJob::fetchDetailsFromRedis()
{
    ...
    try {
        ...
        query_res = opt.hmget((key_name_tmpl % jobType % s_id).str(), "uid", "cid", "post_time", "have_accepted",
                              "no_store_ac_code", "is_rejudge");
    } catch (const RedisNilException & e) {
        LOG_FATAL(0, s_id, log_fp, "job doesn't exist. Exception infomation: ", e.what());
        throw;
    } catch (const RedisUnexpectedCaseException & e) {
        LOG_FATAL(0, s_id, log_fp, "redis returns an unexpected type. Exception infomation: ", e.what());
        throw;
        ...
        LOG_FATAL(0, s_id, log_fp, "job doesn't exist. Exception infomation: ", UNKNOWN_EXCEPTION_WHAT);
        throw;
    }
    ...
    try {
        ...
        LOG_FATAL(0, s_id, log_fp, "job details lost or type cast failed. Exception infomation: ", e.what());
        throw;
    } catch (...) {
        LOG_FATAL(0, s_id, log_fp, "job details lost or type cast failed. Exception infomation: ",
...
int UpdatorJob::calculate_similarity() const
{
    ...
    switch (this->lang) {
        case Language::C:
            sim_cmd = "sim_c -S -P Main.c / %s/%d/0/* > sim.txt"_fmt(accepted_solutions_dir, this->p_id).str();
            break;
        case Language::Cpp:
        case Language::Cpp14:
            sim_cmd = "sim_c -S -P Main.cpp / %s/%d/0/* > sim.txt"_fmt(accepted_solutions_dir, this->p_id).str();
            break;
        case Language::Java:
            sim_cmd = "sim_java -S -P Main.java / %s/%d/2/* > sim.txt"_fmt(accepted_solutions_dir, this->p_id).str();
            break;
    }
...
// 将已经AC且no_store_ac_code标志不为1的代码存入accepted_solutions_dir目录
    ...
    if (access(code_path.c_str(), W_OK) != 0) {
        LOG_FATAL(jobType, s_id, log_fp, "accepted_solutions_dir does not have write permission");
        throw JobHandleException("accepted_solutions_dir does not have write permission");
    }
    code_path += "/" + std::to_string(p_id);
    if (access(code_path.c_str(), W_OK) != 0) {
        if (mkdir(code_path.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) != 0) {
            LOG_FATAL(jobType, s_id, log_fp, "Cannot create directory: ", code_path);
    ...
    if (dir == nullptr) {
        LOG_FATAL(jobType, s_id, log_fp, "Cannot open directory ", code_path,
    ...
    switch (lang) {
        case Language::C:
            copy_cmd = "cp Main.c %s/%d.c"_fmt(code_path, s_id).str();
            break;
        case Language::Cpp:
        case Language::Cpp14:
            copy_cmd = "cp Main.cpp %s/%d.cpp"_fmt(code_path, s_id).str();
            break;
        case Language::Java:
            copy_cmd = "cp Main.java %s/%d.java"_fmt(code_path, s_id).str();
            break;
    }
    ...
    if (-1 == copy_cmd_return_value) {
        LOG_FATAL(jobType, s_id, log_fp, "Fork failed while copying code to accepted_solutions_dir");
    ...
    if (!WIFEXITED(copy_cmd_return_value) || WEXITSTATUS(copy_cmd_return_value)) {
        LOG_FATAL(jobType, s_id, log_fp, "Error occurred while copying code to accepted_solutions_dir, exit status: ",
...
Result UpdatorJob::get_job_result()
{
    try {
        ...
        Operation opt(redisConn);
        query_res = opt.hmget((judge_status % jobType % s_id).str(), "result", "cpu_time", "memory");
        ...
        LOG_FATAL(0, s_id, log_fp, "job result doesn't exist. Exception infomation: ", e.what());
        throw;
    } catch (const std::exception & e) {
        LOG_FATAL(0, s_id, log_fp, "job result doesn't exist. Exception infomation: ", e.what());
        throw;
    } catch (...) {
        LOG_FATAL(0, s_id, log_fp, "job result doesn't exist. Exception infomation: ", "unknown exception");
        throw;
    }
}
...
void UpdatorJob::update_solution(const Result & result)
{
    try {
        ...
        LOG_FATAL(jobType, s_id, log_fp, "Update solution failed! Error information: ", e.what());
        throw;
    } catch (const mysqlpp::BadQuery & e) {
        LOG_FATAL(jobType, s_id, log_fp, "Update solution failed! Error information: ", e.what());
        throw;
    } catch (const MysqlEmptyResException & e) {
        LOG_FATAL(jobType, s_id, log_fp, "Update solution failed! Error information: ", e.what(), " error code: ", e.errnum());
        throw;
    } catch (const std::exception & e) {
        LOG_FATAL(jobType, s_id, log_fp, "Update solution failed! Error information: ", e.what());
        throw;
...
void UpdatorJob::update_exercise_solution(const Result & result)
{
	...
	templ.parse();
	mysqlpp::SimpleResult res = templ.execute(solution_table, s_id, uid, p_id, (int) lang, result.judge_result,
...
void UpdatorJob::update_source_code(const char * source_code)
{
    ...
    query.parse();
    mysqlpp::SimpleResult res = query.execute(source_code_table, s_id, source_code);
    if (!res) {
...
void UpdatorJob::update_compile_info()
{
    ...
    RedisReply reply = get_ce_info_templ.execute(redisConn, jobType, s_id);
    if (reply.replyType() != RedisReplyType::STRING) {
        LOG_FATAL(this->jobType, this->s_id, log_fp, "Unable to get compile info!");
        throw kerbal::redis::RedisUnexpectedCaseException(reply.replyType());
    }

    if (std::strcmp(reply->str, "") == 0) {
        LOG_WARNING(jobType, s_id, log_fp, "Empty compile info string!");
    }
    ...
    try {
        res = insert.execute(compile_info_table, s_id, reply->str);
    } catch (const mysqlpp::BadParamCount & e) {
        LOG_FATAL(jobType, s_id, log_fp, "Update compile_info failed! Error information: ", e.what());
        throw;
    }
    if (!res) {
        LOG_FATAL(jobType, s_id, log_fp, "Update compile_info failed! ",
...
int UpdatorJob::update_user_and_problem(const Result & result)
{
    ...
    try {
        res = query.store(uid, p_id, cid_param);
    } catch (const mysqlpp::BadParamCount & e) {
        LOG_FATAL(jobType, s_id, log_fp, "Select status from user_problem failed! Error information: ", e.what());
        throw;
    }
    if (res.empty()) {
        if (query.errnum() != 0) {
            LOG_FATAL(jobType, s_id, log_fp, "Select status from user_problem failed! ",
    ...
    if (result.judge_result == UnitedJudgeResult::ACCEPTED && already_AC == false) {
        ...
        }
        {
            ...
            mysqlpp::SimpleResult res;
            try {
                res = update.execute(p_id);
            } catch (const mysqlpp::BadParamCount & e) {
                LOG_WARNING(jobType, s_id, log_fp, "Update problem failed! Error information: ", e.what());
                throw;
            }
            if (!res) {
                LOG_WARNING(jobType, s_id, log_fp, "Update problem failed! ",
                            "Error code: ", update.errnum(),
                            ", Error information: ", update.error());
                throw std::logic_error(update.error());
            }
...
int UpdatorJob::update_course_user(const Result & result)
{
    ...
    sprintf(sql, "select status from user_problem "
                 "where u_id= %d and p_id = %d and c_id = %d and status = '0'", uid, p_id, cid);
    ...
    if (res == NULL) {
        LOG_FATAL(jobType, s_id, log_fp, "store result failed!, errno:%d, error:%s\nsql:%s", mysql_errno(conn),
    ...
    if (result.judge_result == UnitedJudgeResult::ACCEPTED && !already_AC) {
        ...
        if (ret) {
            LOG_WARNING(jobType, s_id, log_fp, "update course_user failed!,errno:%d, error:%s", mysql_errno(conn),
                        mysql_error(conn));
            return ret;
        }
    } else if (!already_AC) {
...
int UpdatorJob::update_user_problem(int stat)
{
    ...
    if (cid) {
        sprintf(sql, "select status from user_problem "
                     "where u_id = %d and p_id = %d and c_id = %d", uid, p_id, cid);
    } else {
        sprintf(sql, "select status from user_problem "
                     "where u_id = %d and p_id = %d and c_id is NULL", uid, p_id);
    }
    int ret = mysql_real_query(conn, sql, strlen(sql));
    if (ret) {
        LOG_WARNING(jobType, s_id, log_fp, "select status from user_problem failed!, errno:%d, error:%s\nsql:%s",
                    mysql_errno(conn), mysql_error(conn), sql);
        return ret;
    }
    res = mysql_store_result(conn);
    ...
    if (row == NULL) {
        if (cid) {
            sprintf(sql, "insert into user_problem (u_id, p_id, c_id, status)"
                         "values ('%d', '%d', '%d', '%d')", uid, p_id, cid, stat);
        } else {
            sprintf(sql, "insert into user_problem (u_id, p_id, status)"
                         "values ('%d', '%d', '%d')", uid, p_id, stat);
        }
    } else if (!stat && atoi(row[0])) {
        if (cid) {
            sprintf(sql, "update user_problem set status = %d "
                         "where u_id = %d and p_id = %d and c_id = %d", stat, uid, p_id, cid);
        } else {
            sprintf(sql, "update user_problem set status = %d "
                         "where u_id = %d and p_id = %d and c_id is NULL", stat, uid, p_id);
        }
    } else {
        mysql_free_result(res);
        return 0;
    ...
    ret = mysql_real_query(conn, sql, strlen(sql));
    if (ret) {
        LOG_WARNING(jobType, s_id, log_fp, "update user_problem failed!, errno:%d, error:%s\nsql:%s", mysql_errno(conn),
                    mysql_error(conn), sql);
    }
    return ret;
}
...
int UpdatorJob::update_contest_user_problem(int is_ac)
{
    ...
    // is first AC
    memset(sql, 0, sizeof(sql));
    sprintf(sql, "select first_ac_user from contest_problem where ct_id = %d and p_id = %d", jobType, p_id);
    ...
    res = mysql_store_result(conn);
    if (res == NULL) {
        LOG_FATAL(jobType, s_id, log_fp, "store result failed! res is NULL, errno:%d, error:%s\nsql:%s",
                  mysql_errno(conn), mysql_error(conn), sql);
        return mysql_errno(conn);
    }
    row = mysql_fetch_row(res);
    if (row == NULL) {
        LOG_FATAL(jobType, s_id, log_fp, "cannot find record accordingly, maybe that problem has been deleted.\nsql:%s",
                  sql);
        return -1;
    }
    ...
    // is already AC
    memset(sql, 0, sizeof(sql));
    sprintf(sql, "select is_ac from contest_user_problem%d where u_id = %d and p_id = %d", jobType, uid, p_id);
    ...
    if (row == NULL) {
        ...
        sprintf(sql, "insert into contest_user_problem%d (u_id, p_id, is_ac, ac_time, error_count, is_first_ac) "
                     "values ('%d', '%d', '%d', '%s', '%d', '%d')", jobType, uid, p_id, is_ac, postTime.c_str(),
        ...
        if (tmp_row == NULL) {
            LOG_FATAL(jobType, s_id, log_fp, "impossible! get error_count failed!, errno:%d, error:%s\nsql:%s",
                      mysql_errno(conn), mysql_error(conn), sql);
            return -1;
        }
    ...
    ret = mysql_real_query(conn, sql, strlen(sql));
    if (ret) {
        LOG_WARNING(jobType, s_id, log_fp, "update contest_user_problem%d failed!, errno:%d, error:%s\nsql:%s", jobType,
                    mysql_errno(conn), mysql_error(conn), sql);
        return ret;
    }
    ...
    if (is_ac && is_first_ac) {
        memset(sql, 0, sizeof(sql));
        sprintf(sql, "update contest_problem set first_ac_user = %d where ct_id = %d and p_id = %d", uid, jobType, p_id);

        ret = mysql_real_query(conn, sql, strlen(sql));
        if (ret) {
            LOG_WARNING(jobType, s_id, log_fp, "update first_ac_user failed!, errno:%d, error:%s\nsql:%s",
                        mysql_errno(conn), mysql_error(conn), sql);
            return ret;
        }
    }
...
int UpdatorJob::core_update_failed_table(const Result & result) noexcept
{
    ...
    RedisReply reply = get_src_code_templ.execute(redisConn, this->jobType, this->s_id);
    ...
    RedisReply reply2 = get_ce_info_templ.execute(redisConn, this->jobType, this->s_id);
    ...
    sprintf(sql,
            ...
            jobType, s_id, p_id, uid, (int) lang, postTime.c_str(), cid, cases, timeLimit.count(), timeLimit, memoryLimit,
